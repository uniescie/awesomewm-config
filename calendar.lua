...
function cal.register(mywidget, custom_current_day_format)
    ...
    if not tooltip then
        ...
        function tooltip:update()
            ...
            state = {month, year}
            tooltip:set_markup(string.format('<span font_desc="monospace" color="#DCDCCC">%s</span>', displayMonth(month, year, 2)))
        end
        tooltip:update()
    end

    -- calendar_format = string.format('<span color="#FFFFFF" background="#000000">%%s</span>')
    -- tooltip:add_to_object(mywidget, calendar_format)
